...
/*   Updated: 2021/02/19 16:48:57 by mwinter          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_word_in_quotes(char *line, t_all *all, char **res)
{
	while (line[all->i] && line[all->i] != '\'')
	{
...
char		*get_in_quotes(char *line, t_all *all)
{
	...
	while (line[all->i])
	{
		if (q == '\'' && get_word_in_quotes(line, all, &res))
			break ;
